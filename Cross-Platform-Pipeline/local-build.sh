...
docker run -e QEMU_TCG_ACCELERATOR="${QEMU_TCG_ACCELERATOR:-}" -v $(pwd)/smoke-test/job:/job --name qemu-vm-container --hostname qemu-vm-container --device /dev/fuse --cap-add SYS_ADMIN --security-opt apparmor:unconfined -t devizervlad/crossplatform-pipeline:${QEMU_IMAGE_ID} bash -e -c "echo; bash -e act.sh"
