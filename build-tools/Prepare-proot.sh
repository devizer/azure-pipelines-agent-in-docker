...
cat <<-'EOF' > /tmp/provisioning-$KEY
  ...
  if true || [[ "$os_ver" != "debian:7" ]]; then
    ...
    export INSTALL_DIR=/usr/local TOOLS="bash git jq 7z nano gnu-tools cmake"; script="https://master.dl.sourceforge.net/project/gcc-precompiled/build-tools/Install-Build-Tools.sh?viasf=1"; (wget -q -nv --no-check-certificate -O - $script 2>/dev/null || curl -ksSL $script) | bash
  else
