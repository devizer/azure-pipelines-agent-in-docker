...
Test1() {
  ...
  local tmp="$(Get_Tmp_Folder)"
  Download_File_Failover "$tmp/$file" "$url"
  sz="$(ls -l $tmp/$file | awk '{print $5}')"
  DEFINITION_COLOR=Default VALUE_COLOR=LightPurple Say-Definition "$tmp/$file size is" "[$(Format_Thousand "$sz") bytes]"
  rm -f $tmp/$file || true
  echo
}
