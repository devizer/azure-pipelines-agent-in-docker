...
Say-Definition
Say-Definition only_1_parameter
Say-Definition "" "value_for_empty_title"
...
Test1() {
  local file="$1"
  local url="https://sourceforge.net/projects/p7zz-repack/files/windows/$file/download"
  Say-Definition Test download url "'$url'"
  local tmp="$(Get-Tmp-Folder)"
  Download-File-Failover "$tmp/$file" "$url"
  sz="$(ls -l $tmp/$file | awk '{print $5}')"
  DEFINITION_COLOR=Default VALUE_COLOR=LightPurple Say-Definition "$tmp/$file size is" "[$(Format-Thousand "$sz") bytes]"
  rm -f $tmp/$file || true
  echo
}

Colorize Cyan "FIRST: Progress is hidden by default"
DOWNLOAD_SHOW_PROGRESS="" Test1 7z-full-x64-2301.7z 

Colorize Cyan "NEXT: SHOW Progress"
DOWNLOAD_SHOW_PROGRESS=1 Test1 7z-full-arm64-2301.7z 
