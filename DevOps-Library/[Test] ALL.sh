...
Say-Definition "Get-Linux-OS-Bits() =" "'$(Get-Linux-OS-Bits)'"
Say-Definition "Get-Linux-OS-Architecture() =" "'$(Get-Linux-OS-Architecture)'"
...
Say-Definition "Is-Qemu-Process() =" "'$(Is-Qemu-Process)'"
...
Say-Definition "MkTemp-Folder-Smarty(template 3) =" "'$(MkTemp-Folder-Smarty "template 3")'"
Say-Definition "MkTemp-Folder-Smarty(template 4, prefix 4) =" "'$(MkTemp-Folder-Smarty "template 4" "prefix 4")'"
Say-Definition "MkTemp-File-Smarty(file.json) =" "'"$(MkTemp-File-Smarty "file.json")"'"
Say-Definition "MkTemp-File-Smarty(file.json, store) =" "'"$(MkTemp-File-Smarty "file.json" "store")"'"
...
for ea in "" "md5 sha1 sha224 sha256 sha384 sha512"; do
  ...
  Colorize Yellow "EXISTING_HASH_ALGORITHMS='$EXISTING_HASH_ALGORITHMS'; find_hash_algorithm() = '$(Find-Hash-Algorithm)'"
  unset EXISTING_HASH_ALGORITHMS
done

file="$(MkTemp-File-Smarty "Hashing Tests.txt" "DevOps Lib Demo")"
printf "%s" "hash" > "$file"
echo "hash file is '$file'"
for alg in sha512 sha384 sha256 sha224 sha1 md5; do
  title="$(printf "%10s" "${alg} is")"
  Colorize Cyan "$title '$(Get-Hash-Of-File "$alg" "$file")'"
done 
rm -f "$file"
...
for a in "" 1 12 123 1234 12345 123456 12313123123123123123123123123123123123; do
  echo "Format_Thousand($a): «$(Format-Thousand "$a")»"
  echo "Format_Thousand("$a."): «$(Format-Thousand "$a.")»"
  echo "Format_Thousand("$a.1"): «$(Format-Thousand "$a.1")»"
  echo "Format_Thousand("$a.2K"): «$(Format-Thousand "$a.2K")»"
done 



printf "%s" "To_Boolean():"
for x in True False 0 1 Enable Disable "" Enabled Disabled; do Colorize --NoNewLine Cyan " '$x'"; printf "=>$(To-Boolean "Arg" "$x")"; done
echo ""

DISABLE_COLOR_OUTPUT=tRue Colorize Green "Fake Green (not a green)"

folder=/usr/bin; [[ "$(Is-Termux)" == True ]] && folder="$PREFIX"
Say-Definition "[$folder] folder size:" "$(Format-Thousand "$(Get-Folder-Size "$folder")")"
du="$(command -v du)"
Say-Definition "[$du] file size:" "$(Format-Thousand "$(Get-File-Size "$du")")"

for arch in 7z zip gz xz 404; do
  DEFINITION_COLOR=Magenta Say-Definition "Find-7z-For-Unpack('$arch') =" "[$(Find-7z-For-Unpack "$arch")]"
done

printf "GLIBC Version: "
Colorize LightPurple "$(Get-Glibc-Version)"

Colorize Green "/usr/bin sha512 hash: $(Get-Hash-Of-Folder-Content sha512 "/usr/bin")"
