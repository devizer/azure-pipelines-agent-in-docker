...
Say-Definition "Get_NET_RID() =" "'$(Get_NET_RID)'"
Say-Definition "Get_Global_Seconds() =" "'$(Get_Global_Seconds)'"
Say-Definition "Is_Qemu_VM() =" "'$(Is_Qemu_VM)'"
Say-Definition "Is_Termux() =" "'$(Is_Termux)'"
Say-Definition "Is_Windows() =" "'$(Is_Windows)'"
Say-Definition "Is_WSL() =" "'$(Is_WSL)'"
...
for ea in "" "md5 sha1 sha224 sha256 sha384 sha512"; do
  ...
  Colorize Yellow "EXISTING_HASH_ALGORITHMS='$EXISTING_HASH_ALGORITHMS'; find_hash_algorithm() = '$(Find_Hash_Algorithm)'"
...
for alg in sha512 sha384 sha256 sha224 sha1 md5; do
  ...
  Colorize Cyan "$title '$(Get_Hash_Of_File "$alg" "$file")'"
done 
rm -f "$file"
...
for a in '123 1 " bytes"' '456 1 " bytes"' '123456 1 " bytes"' '456789 1 " bytes"' '5555555555 1 " bytes"' '3456354345345 2 " words"'; do
  ...
  col_value="$(printf "%s" "$(My_Quote "${formatted}")")"
  echo -e "$col_arg: $col_value"
done

for a in "" 1 12 123 1234 12345 123456 12313123123123123123123123123123123123; do
  Colorize Yellow "Format_Thousand($a): «$(Format_Thousand "$a")»"
done 

printf "%s" "To_Boolean():"
for x in True False 0 1 Enable Disable "" Enabled Disabled; do Colorize --NoNewLine Cyan " '$x'"; printf "=>$(To_Boolean "Arg" "$x")"; done
echo ""
