...
# Clean_Up_My_Temp_Folders_and_Files_on_Exit "$ProductId Setup Temp Files"
...
Download_File_Failover "$versionFile" "$versionUrl"
version="$(cat "$versionFile")"
Say-Definition "Installing or Upgrading S5 Dashboard version" "'$version'"
# rm -f "$versionFile"

for forceGZip in tRue fAlse; do
  FORCE_GZIP_PRIORITY=$forceGZip
  Find_Decompressor
  rid="$(Get_NET_RID)"
  fileNameOnly="sqlinsights-dashboard-$rid.tar.${COMPRESSOR_EXT}"
  if [[ "$(Get_OS_Platform)" == Windows ]]; then fileNameOnly="sqlinsights-dashboard-$rid.${COMPRESSOR_EXT}"; fi
  ...
  Fetch_Distribution_File "S5 Dashboard" "$fileNameOnly" "$fullFileName" "$hashDictionaryUrl" "$fileUrl"
  
  extractTo="$(Get_Tmp_Folder)/Extracted $fileNameOnly"
  echo "Extracting [$fullFileName] --> '$extractTo'"
  Extract_Archive "$fullFileName" "$extractTo" --Reset-Target-Folder
  ...
  Validate_File_Is_Not_Empty "$probeFile" "EXTRACT SUCCESS. File Exists: %s. Size is" "EXTRACT ERROR. File Not Found: $probeFile"
